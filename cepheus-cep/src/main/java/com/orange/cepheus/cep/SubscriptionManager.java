...
/**
 * SubscriptionManager manage subscriptions of EventTypeIn to provider When a
 * configuration is loaded, SubscriptionManager send subscription to every
 * provider Every five minutes SubscriptionManager verify if subscription is
 * valid
 */
@Component()
public class SubscriptionManager {
	...
	/**
	 * Periodicity of the subscription task. Default: every 5 min. Must be
	 * smaller than the subscription duration !
	 */
	...
	/**
	 * Update subscription to new provider of the incoming events defined in the
	 * Configuration
	 *
	 * @param configuration
	...
	public void setConfiguration(Configuration configuration) {
		...
		// this prevents active subscription tasks to add subscription ids from
		// the previous configuration
		// this will also copy the subscription information of the previous
	...
	/**
	 * Check that a given subscription is valid unsubscribe if is invalid
	 *
	 * @param subscriptionId
	 *            the id of subscription
	 * @return true if subscription is valid
	 */
	public boolean validateSubscriptionId(String subscriptionId,
			String originatorUrl) {
		if (validateSubscriptionsId) {
			boolean isValid = subscriptions.isSubscriptionValid(subscriptionId);
			if (!isValid) {
				logger.warn(
						"unsubscribeContext request: clean invalid subscription id {} / {}",
						subscriptionId, originatorUrl);
				// TODO: add support multi-tenant subscription

				ngsiClient.unsubscribeContext(originatorUrl, null,
						subscriptionId).addCallback(
						unsubscribeContextResponse -> logger.debug(
								"unsubscribeContext completed for {}",
								originatorUrl),
						throwable -> logger.warn(
								"unsubscribeContext failed for {}",
								originatorUrl, throwable));
	...
	public void shutdownGracefully() {
		...
		eventTypeIns.forEach(eventTypeIn -> eventTypeIn.getProviders().forEach(
		...
		try {
			...
			logger.warn("Failed to shutdown gracefully NGSI pending requests",
					e);
		}
	}

	/**
	 * Cancel any previous schedule, run subscription task immediately and
	 * schedule it again.
	 */
	private void scheduleSubscriptionTask() {
		...
		scheduledFuture = taskScheduler.scheduleWithFixedDelay(
				this::periodicSubscriptionTask, subscriptionPeriodicity);
	}

	private void periodicSubscriptionTask() {

		Instant now = Instant.now();
		Instant nextSubscriptionTaskDate = now
				.plusMillis(subscriptionPeriodicity);
		logger.info("Launch of the periodic subscription task at {}",
				now.toString());
		...
		for (EventTypeIn eventType : eventTypeIns) {
			...
			for (Provider provider : eventType.getProviders()) {
				...
				if (subscriptionDate != null) {
					Instant subscriptionEndDate = subscriptionDate
							.plus(Duration.parse(subscriptionDuration));
					...
					if (nextSubscriptionTaskDate.compareTo(subscriptionEndDate) >= 0) {
						...
						// if delay is passed then clear the subscription info
						// in provider et suppress subscription
				...
				// Send subscription if subscription is a new subscription or we
				// do not receive a response (subscriptionDate is null)
				// Send subscription if deadline is passed
				if ((subscriptionDate == null) || deadlineIsPassed) {
					// lazy build body request only when the first request
					// requires it
	 ...
	 * @param subscriptions
	 */
	private void subscribeProvider(Provider provider,
			SubscribeContext subscribeContext, Subscriptions subscriptions) {
		logger.debug("Subscribe to {} for {}", provider.getUrl(),
				subscribeContext.toString());
		// adeeb added code for checking provider service name and service path

		if (StringUtils.isNotEmpty(provider.getServiceName())
				|| StringUtils.isNotEmpty(provider.getServicePath())) {
			httpHeaders = getHeadersForProvider(provider);

		}
		// change httpheader value from null
		ngsiClient
				.subscribeContext(provider.getUrl(), httpHeaders,
						subscribeContext)
				.addCallback(
						subscribeContextResponse -> {
							SubscribeError error = subscribeContextResponse
									.getSubscribeError();
							if (error == null) {
								String subscriptionId = subscribeContextResponse
								...
								logger.debug("Subscription done for {}",
										provider.getUrl());
							} else {
								logger.warn(
										"Error during subscription for {}: {}",
										provider.getUrl(), error.getErrorCode());
							}
						},
						throwable -> {
							logger.warn("Error during subscription for {}",
									provider.getUrl(), throwable);
						});
	}
...
//Adeeb change null to httpheader
	...
	private SubscribeContext buildSubscribeContext(EventTypeIn eventType) {
		...
		EntityId entityId = new EntityId(eventType.getId(),
				eventType.getType(), eventType.isPattern());
		subscribeContext.setEntityIdList(Collections.singletonList(entityId));
		subscribeContext.setAttributeList(eventType.getAttributes().stream()
	...
	/**
	 * Migrate subscriptions from previous configuration to the new
	 * configuration
	 *
	 * @param configuration
	 *            the new configuration where the subscriptions must be
	 *            insterted
	...
	private Subscriptions migrateSubscriptions(Configuration configuration) {
		...
		// For every previous eventType, find the corresponding one in new
		// configuration
		eventTypeIns
				.forEach(oldEventTypeIn -> {
					Optional<EventTypeIn> maybeEventTypeIn = configuration
							.getEventTypeIns().stream()
							.filter(e -> e.equals(oldEventTypeIn)).findFirst();
					if (maybeEventTypeIn.isPresent()) {
						EventTypeIn newEventTypeIn = maybeEventTypeIn.get();

						// For every previous provider, find the corresponding
						// one in new configuration
						oldEventTypeIn
								.getProviders()
								.forEach(
										oldProvider -> {

											Optional<Provider> optionalProvider = newEventTypeIn
													.getProviders()
													.stream()
													.filter(p -> p.getUrl()
															.equals(oldProvider
											...
											if (optionalProvider.isPresent()) {
												...
												provider.setSubscriptionId(oldProvider
														.getSubscriptionId());
												provider.setSubscriptionDate(oldProvider
														.getSubscriptionDate());
												newSubscriptions
														.addSubscription(oldProvider
																.getSubscriptionId());
											} else {
												// Provider not found in new
						...
						// EventType not found in new configuration, unsubscribe
						// from all providers
						oldEventTypeIn.getProviders().forEach(
								this::unsubscribeProvider);
					}
				});
